...
Launcher.prototype.run = function (args) {
    var thisLauncher, searchFilters;
    
    thisLauncher = this;
    searchRoot = process.cwd();
    searchFilters = [
        function (fileName, fileStat) {
    ...
    this._findFiles(searchRoot, searchFilters, function (error, files) {
        var report, onValidateFileCount, onValidateFile;
        if (error) {
...
Launcher.prototype._findFiles = function (searchRoot, searchFilters, callback) {
    ...
    searchFilter = function (result, rootPath, filters) {
        var files = [], i;
        rootPath = rootPath || '/';
        if (! Array.isArray(result)) {
            if (result.stat.isFile()) {
                for (i = 0; i < filters.length; i += 1) {
                    if(! filters[i](rootPath, result.stat)) {
                        return files;
                    }
                } 
                files.push(rootPath);
                ...
                result.children.forEach(function (child) {
                    files = files.concat(searchFilter(child, path.join(rootPath, child.name), filters));
                });
            }
        } else {
            result.forEach(function (file) {
                files = files.concat(searchFilter(file, path.join(rootPath, file.name), filters));
            });
        }
        return files;
    };
            ...
            if (stat.isDirectory()) {
                __readDirectory(searchRoot, function (error, result) {
                    if (error) {
                        ...
                        if (result) {
                            files = searchFilter(result, searchRoot, searchFilters);
                        }
                        console.log(files);
