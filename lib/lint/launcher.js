...
Launcher.prototype.configure = function (options) {
    if (options) {
        ...
        if (options.formatter !== undefined) {
            ...
            if (this._colors === 'auto') {
                options.formatter.colors = this._guessColors();
            } 
    ...
    onFileFound = function (error, filesFound) {
        ...
        if (onFileFoundCount <= 0) {
            thisLauncher.validateFiles(searchResult, function (error, report) {
                thisLauncher.printMessage(thisLauncher._formatReport(report), function (error, result) {
...
if (__filename === process.ARGV[1]) {
    ...
    if (configFile) {
        ...
        //Parse config
        try {
            launcher.configure(JSON.parse(source));
        } catch (e) {
